...
const AUTH_BRIDGE_URL='https://script.google.com/macros/s/AKfycbwpvABCx0YzyQukvKwkuJqW_k4rkycwyrgkcuXVcJ5v8D64aS9g8h3pQksVNdX6lrf7BQ/exec';
...
const auth={allowed:false,idToken:'',checking:false,blocked:false,error:'',bridgeReady:false,bridgeOrigin:'',gisReady:false};
const authRequests=new Map();
let resolveAuthBridgeReady;
const authBridgeReadyPromise=new Promise(resolve=>{resolveAuthBridgeReady=resolve;});
...
function ensureAuthBridge(){
 if(document.getElementById('participation-auth-bridge'))return;
 const frame=document.createElement('iframe');
 frame.id='participation-auth-bridge';
...
function waitForAuthBridge(){
 if(auth.bridgeReady)return Promise.resolve();
 return Promise.race([authBridgeReadyPromise,new Promise((_,reject)=>setTimeout(()=>reject(new Error('bridge_timeout')),12000))]);
}
async function callAuthBridge(action,idToken){
 await waitForAuthBridge();
 const frame=document.getElementById('participation-auth-bridge');
 if(!frame?.contentWindow||!auth.bridgeOrigin)throw new Error('bridge_unavailable');
 const requestId=authRequestId();
 return new Promise((resolve,reject)=>{
  const timer=setTimeout(()=>{authRequests.delete(requestId);reject(new Error('request_timeout'));},20000);
  authRequests.set(requestId,{resolve:result=>{clearTimeout(timer);resolve(result);}});
  frame.contentWindow.postMessage({channel:AUTH_CHANNEL,type:'request',requestId,action,idToken},auth.bridgeOrigin);
 });
}
window.addEventListener('message',event=>{
 ...
 if(message.type==='ready'){
  auth.bridgeOrigin=event.origin;
  if(!auth.bridgeReady){auth.bridgeReady=true;resolveAuthBridgeReady();}
  return;
 }
