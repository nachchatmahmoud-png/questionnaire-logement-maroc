...
const G={
 information:[
  ['الوصول إلى المعلومات',[['info_1','1. يسهل التعرف على القنوات الرسمية التي تنشر المعلومات المتعلقة ببرنامج الدعم المباشر للسكن.'],['info_2','2. يسهل الوصول إلى المعلومات المتعلقة بالبرنامج عبر وسائل التواصل الرسمية للوزارة.'],['info_3','3. أجد المعلومات الرسمية المتعلقة بالبرنامج متاحة عندما أحتاج إليها.']]],
...
function requiredIds(step){
 ...
 if(step==='interaction'){let ids=['contact_reel'];if(val('contact_reel')==='نعم'){ids.push('contact_reason','canal_dernier_contact','reponse_recue');if(val('canal_dernier_contact')===OTHER_CONTACT_CHANNEL)ids.push('contact_channel_other_detail');if(val('reponse_recue')==='نعم')ids.push(...G.response[0][1].map(r=>r[0]));}if(val('contact_reel')==='لا'){ids.push('noninteraction_reason');if(route()==='official')ids.push(...interactionPerceptionIds());}return ids;}
